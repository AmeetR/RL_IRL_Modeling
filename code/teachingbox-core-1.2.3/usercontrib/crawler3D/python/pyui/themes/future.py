...
class FutureTheme(ThemeBase):
    ...
    def drawDot(self, rect, color = None):
        if not color:
            color = self.fgColor
        self.renderer.drawRect(color, rect)
...
#            self.renderer.drawLine( rect[0] + i*interval, half-4, rect[0] + i*interval, half+4, pyui.colors.black)        
        ...
        #self.renderer.drawText("%s/%s" % (position,range), (rect[2] - 80, rect[1] - 20), pyui.colors.white)
        self.renderer.drawText("%s/%s" % (position,range), (rect[0], rect[1] - 6), pyui.colors.white)
