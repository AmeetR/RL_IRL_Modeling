/**
 *
 * $Id: StepLengthAveragerForEachDimension.java 696 2010-06-21 11:38:29Z twanschik $
 *
 * @version   $Rev: 696 $
...
/**
 * Averages the step length in each dimension (for each i ||(state.get(i) - nextState.get(i)|| )
 * keeping in mind that we may have to adjust some step lengths (see
 * WrapAdjustedStepLengthAverager or MathUtils.shortestDistance for more info).
 */

public class StepLengthAveragerForEachDimension extends DoubleMatrix1DAverager {
     ...
     * @param dimension the dimension of the vector we are averaging
     * @param configString the config string for plotting
     */
    public StepLengthAveragerForEachDimension(int[] dimensionsToAdjust,
            double[][] boundaries, int maxSteps, String configString, int dimension) {
