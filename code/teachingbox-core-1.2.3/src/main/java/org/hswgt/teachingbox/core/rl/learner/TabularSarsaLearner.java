/**
 *
 * $Id: TabularSarsaLearner.java 380 2009-11-25 14:19:44Z bystrito $
 *
 * @version   $Rev: 380 $
 * @author    $Author: bystrito $
 * @date      $Date: 2009-11-25 15:19:44 +0100 (Mi, 25 Nov 2009) $
 *
 */
...
/**
 * Ordinary Tabular Sarsa-Learning
 * TODO: Add eligibility traces
 */
public class TabularSarsaLearner implements Learner
{
    private static final long serialVersionUID = 444487333654199521L;
    
    // Logger
    private final static Logger log4j = Logger.getLogger("TabularSarsaLearner");
...
//    protected double lambda = DEFAULT_LAMBDA;
     ...
     */
    public TabularSarsaLearner(TabularQFunction Q)
    {
        this.Q = Q;
     ...
     * @param lambdaCalculator The lambda-parameter method to use.
     */
    public TabularSarsaLearner(TabularQFunction Q,
    ...
    protected double getTdError(State s, Action a, State sn, Action an, double r, boolean isTerminalState)
    {
        ...
        if ( !isTerminalState ) 
        {
        	// Sarsa uses the next action (on policy) to perform an update
        	qn = Q.getValue(sn,an);
        }
            ...
            double reward, boolean isTerminalState)
    {
        ...
        //this.notify(tderror, reward, state, action, nextState, nextAction, isTerminalState);
        ...
        */
        ...
        // is the action chosen the best action?
        //boolean bestAction = (Q.getValue(sn,an) == Q.getMaxValue(sn));
