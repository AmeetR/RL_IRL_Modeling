...
/**
 * Renderer of the grid table. Use the GridTableCell to override the standard
 * renderer.
 */
public class GridTableCellRenderer extends GridTableCell implements
		TableCellRenderer, Serializable {

	/**
	 * 
	 */
	private static final long serialVersionUID = 7082182057584730917L;
	...
	private Color terminalStateColor = new Color(244,186,186);
	
	private Color startStateColor = new Color(187,241,184);
	...
	public Component getTableCellRendererComponent(JTable table, Object value,
		...
		if (isSelected) {
			...
			if(GridModel.getInstance().getCell(column, row).isTerminalState()){
				this.setBackground(terminalStateColor);
				this.setFieldBackground(terminalStateColor);
			}else if(GridModel.getInstance().getCell(column, row).isStartState()){
				this.setBackground(startStateColor);
				this.setFieldBackground(startStateColor);
			}else{
